...
#sandbox
# deploy in server
ecs-deploy --max-definitions 3 --enable-rollback -c $CLUSTER -r $REGION -n $TASK_SANDBOX_SERVER -i $IMAGE_SERVER:$TRAVIS_TAG --timeout $TIMEOUT
# deploy in worker
ecs-deploy --max-definitions 3 --enable-rollback -c $CLUSTER -r $REGION -n $TASK_SANDBOX_WORKER -i $IMAGE_WORKER:$TRAVIS_TAG --timeout $TIMEOUT

#live
# deploy in server
ecs-deploy --max-definitions 3 --enable-rollback -c $CLUSTER -r $REGION -n $TASK_LIVE_SERVER -i $IMAGE_SERVER:$TRAVIS_TAG --timeout $TIMEOUT
# deploy in worker
ecs-deploy --max-definitions 3 --enable-rollback -c $CLUSTER -r $REGION -n $TASK_LIVE_WORKER -i $IMAGE_WORKER:$TRAVIS_TAG --timeout $TIMEOUT
